 ...
 * @LastEditor: sunzhguy
 * @LastEditTime: 2020-06-09 08:36:53
 */ 
				...
				int schedPolicy, int schedPriority )
{
	...
	if ( nRet != 0 ) FiPrint( "pthread_attr_init error .\r\n" );
	
	nRet = pthread_attr_setschedpolicy( &attr, schedPolicy );
	if ( nRet != 0 ) FiPrint( "pthread_attr_setschedpolicy error .\r\n" );
	...
	if ( schedPolicy == SCHED_FIFO || schedPolicy == SCHED_RR )
	{
		...
		if ( nRet != 0 ) FiPrint( "pthread_attr_setschedparam error .\r\n" );
	}
	
	nRet = pthread_create( threadId, &attr, threadFunc, arg );
	if ( nRet != 0 ) FiPrint( "pthread_create error .\r\n" );
